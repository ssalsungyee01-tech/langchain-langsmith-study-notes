# DB, DBMS, Vector DB, RAG: LangChain에서 DB는 어떻게 쓰일까

RAG를 배우기 전에 DB라는 말부터 정리하면 훨씬 편합니다. LangChain 수업에서 DB는 한 가지 의미로만 쓰이지 않습니다. 어떤 때는 일반 데이터베이스를 뜻하고, 어떤 때는 vector DB를 뜻하고, 어떤 때는 대화 기록이나 실행 기록을 저장하는 저장소를 뜻합니다.

DB는 database, 즉 데이터를 모아두는 저장소입니다. 고객 목록, 주문 내역, 회의록, 문서 조각, 임베딩 벡터, 대화 기록 같은 것들이 DB에 저장될 수 있습니다.

DBMS는 database management system입니다. DB를 만들고, 저장하고, 검색하고, 수정하고, 권한을 관리하는 프로그램입니다. SQLite, PostgreSQL, MySQL 같은 이름을 들어봤다면 대부분 DBMS입니다. DB가 "자료가 모인 창고"라면, DBMS는 그 창고를 관리하는 시스템입니다.

> #### 이게 뭔데? DB와 DBMS 차이
> DB는 데이터 자체 또는 데이터 저장 공간을 가리키는 말이고, DBMS는 그 데이터를 관리하는 프로그램입니다. 일상에서는 둘을 섞어 말하는 경우가 많지만, 개념적으로는 구분해두면 좋습니다.
...
RAG는 이 중 두 번째와 가장 깊게 연결됩니다. RAG는 모델이 모르는 외부 문서를 검색해서 답변 근거로 넣어주는 방식입니다.
...
[이전 글](05_Tool_Calling.md) · [다음 글: Memory, Thread, State](07_Memory_Thread_State.md)
