...
| Row | 표 안의 실제 데이터 한 줄 |
| Column | 표 안의 데이터 항목 |
| Primary Key | 한 줄을 정확히 구분하는 대표값 |
| Query | DB에 던지는 질문이나 요청 |
| Index | 데이터를 더 빨리 찾기 위한 목차 같은 구조 |
| Transaction | 중간에 끊기면 안 되는 DB 작업 묶음 |
| Data Integrity | 데이터가 틀리거나 서로 모순되지 않는 상태 |
| Entity | DB에 저장하고 싶은 대상 |
| Attribute | 대상이 가진 속성 또는 컬럼 |
| Relationship | 대상들 사이의 연결 |
| Foreign Key | 다른 테이블의 데이터를 가리키는 값 |
| 관계형 DB | 테이블 구조와 관계를 중심으로 데이터를 관리하는 DB |
| 비관계형 DB | 문서, 키-값, 그래프, 벡터 등 다양한 형태로 데이터를 저장하는 DB |
| SQL | 관계형 DB에서 자주 쓰는 조회 언어 |
| NoSQL | 비관계형 DB 계열을 넓게 부르는 말 |
